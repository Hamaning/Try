...
void creer_fichier(Fichier *fichier) {
    ...
    printf("Choisissez l'organisation globale (0 pour Contiguë, 1 pour Chaînée): ");
    scanf("%d", &fichier->organisation_globale);

    printf("Choisissez l'organisation interne (0 pour Non Triée, 1 pour Triée): ");
...
void rechercher_enregistrement(Fichier *fichier, int id) {
    for (int i = 0; i < fichier->nb_enregistrements; i++) {
        if (fichier->blocs[i] != NULL && fichier->blocs[i]->enreg.id == id) {
            printf("Enregistrement trouvé: ID = %d, Données = %s\n", fichier->blocs[i]->enreg.id, fichier->blocs[i]->enreg.data);
            return;
        }
    }
...
void modifier_enregistrement(Fichier *fichier, int id, char *new_data) {
    for (int i = 0; i < fichier->nb_enregistrements; i++) {
        if (fichier->blocs[i] != NULL && fichier->blocs[i]->enreg.id == id) {
            ...
            printf("Enregistrement modifié: ID = %d, Données = %s\n", fichier->blocs[i]->enreg.id, fichier->blocs[i]->enreg.data);
            return;
        }
    }
    printf("Enregistrement non trouvé.\n");
...
void supprimer_enregistrement(Fichier *fichier, int id) {
    for (int i = 0; i < fichier->nb_enregistrements; i++) {
        if (fichier->blocs[i] != NULL && fichier->blocs[i]->enreg.id == id) {
            ...
            fichier->blocs[i] = NULL;
            printf("Enregistrement supprimé: ID = %d\n", id);
            return;
        }
    }
    printf("Enregistrement non trouvé.\n");
...
void trier_enregistrements(Fichier *fichier) {
    ...
    printf("Les enregistrements ont été triés.\n");
}
...
void defragmenter_fichier(Fichier *fichier) {
    ...
    printf("Le fichier a été défragmenté.\n");
}
...
void afficherEtatMemoire() {
    ...
    for (int i = 0; i < MAX_DATA_SIZE; i++) {
        ...
        if (memoire[i].is_free) {
            ...
            printf("%-8s %2d enreg", memoire[i].fichier_nom, memoire[i].nb_enregistrements);
        }
        printf(" |\n");
        printf("---------------------\n");
    }
...
int main(){
    ...
    while (1) {
        ...
        printf("14. Afficher l'état de la mémoire secondaire\n");
        printf("15. Quitter\n");
        ...
        case 2:
            if (nb_fichiers == 0) {
                printf("Aucun fichier n'a été créé.\n");
            } else {
        ...
        case 4:
        	 if (nb_fichiers == 0) {
                ...
                int index_fichier = choisir_fichier(fichiers, nb_fichiers);
                printf("Entrez l'ID de l'enregistrement à modifier: ");
                scanf("%d", &id);
                printf("Entrez les nouvelles données: ");
                getchar();
